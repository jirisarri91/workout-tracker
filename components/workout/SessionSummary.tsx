...
export function SessionSummary({ session, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    fetch('/api/ai', {
      ...
      body: JSON.stringify({
        prompt: `In one sentence (max 20 words), give a motivating comment on this workout: ${exerciseNames}. Be specific and energetic.`,
      }),
    }).then(async res => {
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
      <div className="bg-white w-full sm:max-w-md rounded-t-3xl sm:rounded-2xl p-6 flex flex-col gap-4 max-h-[80vh] overflow-y-auto">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-bold text-slate-900">Workout Complete 💪</h2>
        ...
        {doneExercises.length > 0 && (
          <div>
            <p className="text-xs font-semibold text-slate-500 uppercase tracking-wide mb-2">Exercises</p>
        ...
        <div className="bg-orange-50 rounded-xl p-3">
          <p className="text-xs font-semibold text-orange-500 mb-1">AI Coach</p>
          {loadingAI ? (
