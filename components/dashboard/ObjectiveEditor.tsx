...
export function ObjectiveEditor() {
  ...
  function scheduleAutoSave(newObjective: string, newStrategy: string) {
    if (saveTimeout.current) clearTimeout(saveTimeout.current);
    saveTimeout.current = setTimeout(() => save(newObjective, newStrategy), 800);
  }

  async function save(obj: string, strat: string) {
    setSaving(true);
    try {
      await fetch('/api/objectives', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ objective_text: obj, strategy_text: strat }),
      });
      await mutate();
    } catch {
      ...
      <div className="flex flex-col gap-3">
        <Textarea
          ...
          onChange={e => {
            ...
            scheduleAutoSave(e.target.value, strategy);
          }}
        />
        <Textarea
          ...
          onChange={e => {
            ...
            scheduleAutoSave(objective, e.target.value);
          }}
        />
